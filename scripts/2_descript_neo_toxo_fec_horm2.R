...
(repeated measures)') +
  ...
  ### 3.2 Univariate stats fecal corticosterone (cort)
    ...
    ## d) Histogram fecal corticosterone
      hist_plot_c <- ggplot(data=fec_horm_neosp_toxo_data_12, 
...
(repeated measures)') +
    ...
    ## f) Histogram of Nat. Log. of fecal corticosterone
      hist_plot_ln_c <- ggplot(data=fec_horm_neosp_toxo_data_12, 
...
############################################################################### 
  ...
  ### 4.1 Descriptive bivariate plots testosterone status by sex
    ## a) Scatter plot testosterone by age by sex  
      fec_T_scatter <- ggplot(fec_horm_neosp_toxo_data_12, aes(x = fecal.age.cat, 
                                              y = testosterone.ng.g, 
                                              shape = poop.state, color = sex)) +
        geom_point(position = position_jitter(h=0.0, w=0.3)) +
        geom_hline(yintercept = round(sd(
          fec_horm_neosp_toxo_data_12$testosterone.ng.g, 
                                          na.rm = T), 2) * 4, 
          linetype="solid", color = "red") +
        annotate(geom="text", label = '4 SD', x = 1, 
                 y = round(sd(fec_horm_neosp_toxo_data_12$testosterone.ng.g, 
                   na.rm = T), 2) * 4, vjust=-1) +
...
by age by sex',
    ...
    ## c) Scatter plot corticosterone by age by sex 
      fec_cort_scatter <- ggplot(fec_horm_neosp_toxo_data_12, 
                                 aes(x = fecal.age.cat, 
                                     y = corticosterone.ng.g,
                                     shape = poop.state, color = sex)) +
        geom_point(position = position_jitter(h=0.0, w=0.3)) +
        geom_hline(yintercept = round(sd(
          fec_horm_neosp_toxo_data_12$corticosterone.ng.g, 
          na.rm = T), 2) * 4, 
          linetype="solid", color = "red") +
        annotate(geom="text", label = '4 SD', x = 1, 
                 y = round(sd(fec_horm_neosp_toxo_data_12$corticosterone.ng.g, 
                              na.rm = T), 2) * 4, vjust=-1) +
...
#********************** Data Inclusion/Exclusion Criteria **********************
  ### 4.2 Remove outliers
    ...
    ## a) Create a copy of full data set (including outliers) for sensitivity 
      # analyses
      fec_horm_neosp_toxo_data_12_full <- fec_horm_neosp_toxo_data_12
      
    ## b) Remove testosterone values > 5 SD 
      #set both the untransformed and log transformed values to NA)
      fec_horm_neosp_toxo_data_12 <- fec_horm_neosp_toxo_data_12_full  %>%
        mutate(testosterone.ng.g = ifelse
               ((fec_horm_neosp_toxo_data_12$testosterone.ng.g >
                 sd(fec_horm_neosp_toxo_data_12$testosterone.ng.g, 
                                        na.rm = T)* 5), 
                 NA, fec_horm_neosp_toxo_data_12$testosterone.ng.g)) %>%
        mutate(testosterone.ng.g.ln = ifelse
               ((fec_horm_neosp_toxo_data_12$testosterone.ng.g >
                   sd(fec_horm_neosp_toxo_data_12$testosterone.ng.g, 
                      na.rm = T)* 5), 
                 NA, fec_horm_neosp_toxo_data_12$testosterone.ng.g.ln))
      
    ## c) Remove corticosterone values > 5 SD 
      #set both the untransformed and log transformed values to NA)
      fec_horm_neosp_toxo_data_12 <- fec_horm_neosp_toxo_data_12  %>%
        mutate(corticosterone.ng.g = ifelse
               ((fec_horm_neosp_toxo_data_12$corticosterone.ng.g >
                   sd(fec_horm_neosp_toxo_data_12$corticosterone.ng.g, 
                      na.rm = T)* 5), 
                 NA, fec_horm_neosp_toxo_data_12$corticosterone.ng.g)) %>%
        mutate(corticosterone.ng.g.ln = ifelse
               ((fec_horm_neosp_toxo_data_12$corticosterone.ng.g >
                   sd(fec_horm_neosp_toxo_data_12$corticosterone.ng.g, 
                      na.rm = T)* 5), 
                 NA, fec_horm_neosp_toxo_data_12$corticosterone.ng.g.ln))
                 
#********************** Data Inclusion/Exclusion Criteria ********************** 


                                    
  ### 4.2 Descriptive bivariate stats testosterone status by sex
    ## a) Sex summary 
  ...
  ### 4.2 Descriptive bivariate stats testosterone status by categorical age
    ...
    ## c) Age by toxo.status 
      T_age_toxo_sum_id <- fec_horm_neosp_toxo_data_12 %>%
        filter(!is.na(testosterone.ng.g)) %>%
        group_by (hy.id, fecal.age.cat) %>%
        summarise (n.Test = sum(!is.na(testosterone.ng.g)),
                   avg.Test = round (mean(testosterone.ng.g, 
    ...
    # summarize age by toxo.status T averaged over individuals
      T_age_toxo_sum <- T_age_toxo_sum_id %>%
    ...
    ## e) Age by sex by toxo.status
      #summarize age by toxo.status T averaged over individuals
      T_age_sex_toxo_sum <- T_age_toxo_sum_id %>%
        group_by (fecal.age.cat, sex, toxo.status) %>%
        summarise (n.id = sum(!is.na(avg.Test)),
                   avg.T = round (mean(avg.Test, 
                                       na.rm = T),2),
                   stdev.T = round (sd(avg.Test, 
    ...
    ## f) save the data frame of summary stats out as a pdf into output file
      pdf(here('output/T_age_sex_toxo_sum.pdf'), 
          height = 4, width = 5)
      grid.table(T_age_sex_toxo_sum)
      dev.off()
